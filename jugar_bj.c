...
void jugar_blackjack(int cartas[], Jugador jugadores[], int numJugadores) {
    int valor_apuesta_acumulado = 50;
    ...
    for (int i = 0; i < numJugadores; i++) {
        printf("----------------------------------------");
        printf("\nApuesta Inicial de Entrada: USD 50");
    ...
    for (int i = 0; i < numJugadores; i++) {
        ...
        if (strlen(valor) != 1 || (valor[0] != 's' && valor[0] != 'n')) {
            printf("\nEntrada no válida, solo se permite una letra (s/n)\n");
            return;
        }

        if (valor[0] == 'n') {
            printf("%s ha elegido no apostar. Terminando la partida y regresando al menú\n", jugadores[i].nombre);
            jugadores[i].monto_apuesta -= valor_apuesta_acumulado; // Restar monto apostado del saldo disponible
            jugadores[i].dinero_apostado = valor_apuesta_acumulado;  // Guardar el monto fijo de apuesta por iniciar el juego
            jugadores[i].dinero_perdido = valor_apuesta_acumulado;  // Guardar el monto perdido por salir sin jugar
        ...
        if (valor[0] == 's') {
            ...
            do {
                ...
                valor_apuesta_acumulado += valor_apuesta;

                // Validar monto de apuesta
                if (valor_apuesta >= 50 && valor_apuesta <= jugadores->monto_apuesta) {
                    jugadores[i].monto_apuesta -= valor_apuesta_acumulado; // Restar monto apostado del saldo disponible
                    jugadores[i].dinero_apostado = valor_apuesta_acumulado;  // Guardar el monto apostado
                    printf("Apuesta de USD %d realizada\n", valor_apuesta_acumulado);
                    apuestaValida = 1;
                } else {
                    printf("Monto de apuesta inválido. Tu saldo disponible es USD %d\n", jugadores[i].monto_apuesta);
                    valor_apuesta_acumulado = 0;
                }
